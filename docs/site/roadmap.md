...
对屏幕截图做 OCR 后总结。UI 中已预埋 <span class="rb-badge rb-badge-pro">PRO</span> 标识与 <kbd>⌥</kbd><kbd>S</kbd> 快捷键位，接入后只需切换 enabled 态，无需重排界面。
...
## 云端能力（Pro）<span class="rb-badge rb-badge-pro">PRO</span>

托管模型调用、跨设备历史同步等云端能力（BYOK 用户不受影响）。

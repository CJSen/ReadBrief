...
1. 第一行以「翻译：」开头作为标题(其后可跟一句极简说明,如原文主题),标题长度必须为 6-15 个汉字;超过 15 字会被系统截断,少于 6 字判定不合格。
2. 从第二行起直接输出译文正文,使用简体中文,保留原意与语气,不要编号列表、不要额外总结。
3. 不要输出任何思考/推理过程,直接给出译文。`;

export const TRANSLATE_FORMAT_RULE_EN = `Output format (must be followed exactly):
1. Line 1 starts with "Translate: " as the title (optionally followed by a very short note, e.g. the topic); the title must be 6-15 Chinese characters; over 15 is truncated, under 6 is invalid.
2. From line 2 output the translated text directly in Simplified Chinese, preserving meaning and tone; no numbered list, no extra summary.
...
1. 第一行以「问答：」开头作为标题(可跟一句极简说明),标题长度必须为 6-15 个汉字;超过 15 字会被系统截断,少于 6 字判定不合格。
2. 从第二行起直接输出答案正文,像普通 AI 问答一样自由作答,不做额外格式限制,使用与问题一致的语言。`;

export const QA_FORMAT_RULE_EN = `Output format (must be followed exactly):
1. Line 1 starts with "QA: " as the title (optionally followed by a very short note); the title must be 6-15 Chinese characters; over 15 is truncated, under 6 is invalid.
2. From line 2 output the answer freely, just like a normal AI Q&A, with no extra format restrictions, in the same language as the question.`;
...
1. 第一行 = 标题,只写这一行标题,长度必须为 6-15 个汉字;超过 15 字会被系统截断,少于 6 字判定不合格。不要加「标题:」等前缀,不要用 # 号。
...
1. Line 1 = the title only, 6-15 Chinese characters; over 15 is truncated, under 6 is invalid. No prefixes like 'Title:', no '#'.
